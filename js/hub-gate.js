/* ════════════════════════════════════════════════════════════
   MONWOLF TRADER HUB — Access Gate
   Requires ≥ MIN_BALANCE $MONWOLF to unlock the hub.
...
const CFG = window.MonWolfConfig || {};
const MONWOLF_TOKEN  = CFG.TOKEN || '0x8361a59d340466211ad4aB41C09a32e4530a7777';
const MIN_TOKENS   = 1_000_000n;            // 1M $MONWOLF
...
async function getMonWolfBalance(account){
  if (!account) return 0n;
  const data = '0x70a08231' + account.toLowerCase().replace('0x','').padStart(64,'0');
  const hex = await rpcCall('eth_call', [{to: MONWOLF_TOKEN, data}, 'latest']);
...
function injectStyles(){
  if (document.getElementById('mw-gate-style')) return;
  const css = `
#mw-gate-overlay{
...
#mw-gate-overlay.out{animation:gateOut .4s ease-in forwards;}
...
#mw-gate-card > *{position:relative;z-index:1;}
.cg-lock{
...
function buildOverlay(){
  ...
  wrap.id = 'mw-gate-overlay';
  wrap.innerHTML = `
<div id="mw-gate-card">
  <div class="cg-lock">🔒</div>
  <div class="cg-title">GATED ACCESS</div>
  <div class="cg-sub">MonWolf Pack Den · holders only</div>

  <div class="cg-required">
    you need to hold
    <span class="req-num">1,000,000 $MONWOLF</span>
    to unlock the hub
  </div>
  ...
  <div id="cg-connected" style="display:none;">
    ...
    <a class="cg-btn cyan" href="/swap" id="cg-buy">🛒 BUY $MONWOLF</a>
  ...
  <div class="cg-foot">
    Hold 1M+ $MONWOLF to use the trader hub. Live token feed · cross-holder map · subject watch · native on-page swap.
    <br>$MONWOLF: <a href="https://nad.fun/tokens/${MONWOLF_TOKEN}" target="_blank" rel="noopener">nad.fun</a> · <a href="https://dexscreener.com/monad/${MONWOLF_TOKEN}" target="_blank" rel="noopener">chart</a>
  </div>
</div>`;
...
function teardown(){
  const ov = document.getElementById('mw-gate-overlay');
  if (!ov) return;
...
async function verify(account, silent){
  ...
  try{
    const bal = await getMonWolfBalance(account);
...
async function connectAndVerify(){
  ...
  if (!window.ethereum){
    ...
    if (window.MonWolfWallet && typeof window.MonWolfWallet.requireWallet === 'function'){
      window.MonWolfWallet.requireWallet();
    } else {
...
function startRecheck(){
  ...
  recheckTimer = setInterval(async ()=>{
    ...
    if (!ok){
      ...
      clearInterval(recheckTimer);
      document.body.style.overflow = 'hidden';
      const existing = document.getElementById('mw-gate-overlay');
...
window.MonWolfHubGate = { init, verify, getMonWolfBalance, MIN_RAW, MIN_TOKENS, MONWOLF_TOKEN };
