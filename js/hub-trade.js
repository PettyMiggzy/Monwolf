/* ════════════════════════════════════════════════════════════
   MONWOLF HUB — Universal Monad Swap Engine
...
const MONORAIL = {
  ...
  // MonWolf's registered App ID — 1% fee, 100% to treasury (0x4601...).
  // Earns MONWOLF / output-token on every swap through the hub.
  APP_ID: '1176408161625',
};

const RPC_URLS = (typeof window !== 'undefined' && window.location && /(monwolf\.fun|chogi\.xyz)$/i.test(window.location.hostname))
...
async function monorailQuote({from, to, amount, sender, slippageBps, deadlineSec}){
  ...
  const params = new URLSearchParams({
    ...
    source: MONORAIL.APP_ID,                // monwolf App ID → 1% fee → treasury
  });
...
async function executeSwap({from, to, amountHuman, slippageBps, account, onApproveStarted, onApproveConfirmed}){
  ...
  console.log('[mw-swap] initial quote', { hops: q.routes?.[0]?.length, protocols: q.routes?.[0]?.map(s=>s.splits?.[0]?.protocol) });
  ...
  if (fromAddr === NATIVE_ZERO && baseHops >= 2){
    try {
      ...
      if (altHops < baseHops){
        console.log('[mw-swap] using WMON forward route', { hops: altHops });
        q = alt; usedSrc = WMON_ADDRESS_INTERNAL; didWrap = true;
      }
    } catch(e){ console.warn('[mw-swap] forward WMON quote failed', e); }
  }
  ...
  if (toAddr === NATIVE_ZERO && baseHops >= 2){
    try {
      ...
      const altHops = alt.routes?.[0]?.length ?? 99;
      if (altHops < baseHops){
        console.log('[mw-swap] using WMON reverse route + unwrap', { hops: altHops });
        q = alt; usedDst = WMON_ADDRESS_INTERNAL; needsUnwrap = true;
      }
    } catch(e){ console.warn('[mw-swap] reverse WMON quote failed', e); }
  }
  ...
  if (!isNativeFrom){
    ...
    if (current < amountInWei){
      ...
      if (!simA.ok){ console.error('[mw-swap] approve sim failed', simA.error); throw new Error('Approve will fail: ' + explainRevert(simA.error)); }
  ...
  if (!sim.ok){ console.error('[mw-swap] main sim failed', sim.error); throw new Error(explainRevert(sim.error)); }
  ...
  if (needsUnwrap){
    ...
    if (swapRec){
      ...
      }catch(e){ console.warn('[mw-swap] auto-unwrap WMON failed (user has WMON, can unwrap manually)', e); }
    }
  }
...
// Back-compat alias: pages ported from Chogi reference window.ChogiTrade
if (window.MonWolfTrade && !window.ChogiTrade){ window.ChogiTrade = window.MonWolfTrade; }
