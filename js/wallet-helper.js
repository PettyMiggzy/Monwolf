/* Wallet helper — detect Safari/Chrome on iOS where window.ethereum doesn't exist
   and show users a modal with deeplinks that open monwolf.fun inside their wallet's
   in-app browser. Without this people on iPhone can't connect. */
(function () {
  if (window.MonWolfWallet) return; // already loaded
  ...
  function showSheet() {
    ...
    modal.innerHTML =
      '<div id="chogi-wallet-sheet">' +
        ...
        '<div class="sub">Safari can\'t talk to crypto wallets directly.<br>Tap your wallet — it opens monwolf.fun inside it.</div>' +
...
// Back-compat alias: existing pages reference window.MoyakiWallet
if (window.MonWolfWallet && !window.MoyakiWallet){ window.MoyakiWallet = window.MonWolfWallet; }
